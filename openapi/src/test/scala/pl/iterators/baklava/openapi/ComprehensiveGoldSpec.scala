...
/** Gold test for the four Baklava generators (OpenAPI, ts-rest, simple HTML, ts-fetch).
  *
...
class ComprehensiveGoldSpec
  ...
  override def afterAll(): Unit = {
    ...
    // 4. ts-fetch
    val tsfetchDir = new File("target/baklava/tsfetch")
    deleteRecursively(tsfetchDir)
