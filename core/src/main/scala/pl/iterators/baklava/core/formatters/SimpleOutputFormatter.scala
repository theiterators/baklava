    ...
    """
      ...
      List(
        ...
        Some(s"<tr><td><b>BEHAVIOUR</b></td><td>${p.enrichDescriptions
          ...
          .mkString("<br/>")}</td></tr>"),
        Some(
          s"<tr><td><b>STATUS CODES</b></td><td>${statusCodes.mkString("<br/>")}</td></tr>"),
        ...
        Option.when(r.headers.nonEmpty)(s"<tr><td><b>HEADERS</b></td><td>" +
          ...
          s"</td></tr>"),
        r.request.scalaClassOpt.map(s =>
          s"<tr><td><b>REQUEST SCALA TYPE</b></td><td><pre>$s</pre></td></tr>"),
        r.request.minimal.jsonOpt.map(s =>
          s"<tr><td><b>REQUEST MINIMAL JSON</b></td><td><pre>${s.prettyPrint}</pre></td></tr>"),
        r.request.maximal.jsonOpt.map(s =>
          s"<tr><td><b>REQUEST MAXIMAL JSON</b></td><td><pre>${s.prettyPrint}</pre></td></tr>"),
        r.request.schema.map(s =>
          s"<tr><td><b>REQUEST SCHEMA</b></td><td><pre>${printSchema(s).parseJson.prettyPrint}</pre></td></tr>"),
        r.response.scalaClassOpt.map(s =>
          s"<tr><td><b>RESPONSE SCALA TYPE</b></td><td><pre>$s</pre></td></tr>"),
        r.response.minimal.jsonOpt.map(s =>
          s"<tr><td><b>RESPONSE MINIMAL JSON</b></td><td><pre>${s.prettyPrint}</pre></td></tr>"),
        r.response.maximal.jsonOpt.map(s =>
          s"<tr><td><b>RESPONSE MAXIMAL JSON</b></td><td><pre>${s.prettyPrint}</pre></td></tr>"),
        r.response.schema.map(s =>
          s"<tr><td><b>RESPONSE SCHEMA</b></td><td><pre>${printSchema(s).parseJson.prettyPrint}</pre></td></tr>"),
      ).flatten.mkString("\n") +
      "</table>"
  }
