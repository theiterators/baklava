...
class OpenApiFormatterWorker(jsonSchemaToSwaggerSchemaWorker: JsonSchemaToSwaggerSchemaWorker) {
  ...
  def generateOpenApi(routesList: List[EnrichedRouteRepresentation[_, _]]): OpenAPI = {
    ...
    routesList
      ...
      .map { r =>
        components.addSchemas(schemaClassName(r.request.scalaClassOpt.get), swaggerSchema(r.requestJsonSchemaWrapper))
      }
    routesList
      .map(_.routeRepresentation)
      .filter(_.response.scalaClassOpt.isDefined)
      .map { r =>
        components.addSchemas(schemaClassName(r.response.scalaClassOpt.get), swaggerSchema(r.responseJsonSchemaWrapper))
      }
  ...
  private def routeToOperation[Req, Res](route: EnrichedRouteRepresentation[Req, Res]): Operation = {
    val operation = new Operation()
    operation.setSummary(route.routeRepresentation.description)
    operation.setDescription(route.enrichDescriptions.map(_.description).mkString("\n"))
    ...
    route.routeRepresentation.request.minimal.jsonOpt.foreach { _ =>
      ...
      route.routeRepresentation.request.minimal.jsonOpt.foreach { json =>
        ...
        schema.setType(swaggerSchema(route.routeRepresentation.requestJsonSchemaWrapper).getType)
        schema.set$ref(schemaRefName(route.routeRepresentation.request.scalaClassOpt.get))
        mt.setSchema(schema)
      }
        ...
        case (codeOpt, desc) =>
          ...
          if (code.intValue >= 200 && code.intValue < 204) {
            ...
            route.routeRepresentation.response.minimal.jsonOpt.foreach { json =>
              ...
              val schema = new Schema
              schema.setType(swaggerSchema(route.routeRepresentation.responseJsonSchemaWrapper).getType)
              schema.set$ref(schemaRefName(route.routeRepresentation.response.scalaClassOpt.get))
              mt.setSchema(schema)
            }
